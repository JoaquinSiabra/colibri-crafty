...
Crafty.c('PlayerCharacter', {
	init: function() {
		this.requires('Actor, Multiway, Gravity, Collision, spr_player, SpriteAnimation')
		...
		// Cambiamos la animacion si es necesario
		...
		this.bind('NewDirection', function(data) {
			if (data.x > 0) {
				...
				this.stop().animate('PlayerMovingLeft', animation_speed, -1);
			} else { //up and down
				if (isPlaying('PlayerMovingRight'))	{
					this.stop().animate('PlayerMovingRight', animation_speed, -1);
				}else {
					this.stop().animate('PlayerMovingLeft', animation_speed, -1);
				}		
